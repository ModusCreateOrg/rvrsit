...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.chip','game.levels.main').defines(function(){MyGame=ig.Game.extend({boardSize:100,soundRoot:'media/',computerColor:'white',turn:'black',mode:'double',sounds:{badMove:'bad_move.mp3',white:'chip_flip_white.mp3',black:'chip_flip_black.mp3',newGame:'new_game.mp3'},music:[{name:'Truepianos',song:'Truepianos.caff'},{name:'TweakRAM',song:'SnD_TweakRAM.caff'}],init:function(){var me=this;me.initSound();me.initSettings();me.loadLevel(LevelMain);ig.input.initMouse();ig.input.bind(ig.KEY.MOUSE1,'click');if(window.Rvrsit){Rvrsit.app.fireEvent('gameInitialized',me);}
else{window.game=this;}},initSound:function(){var me=this,soundRoot=me.soundRoot,settings=me.getSettings(),fxRoot,musicRoot;if(settings.fx>0&&!me.fxInitialized){fxRoot=soundRoot+'sounds/';var sounds=me.sounds,sound,name;for(name in this.sounds){sound=new ig.Sound(fxRoot+sounds[name],true);sound.volume=settings.fx;sounds[name]=sound;}
...
color=((xIndex+yIndex)%2)?black:white;visible=((xIndex==4||xIndex==5)&&(yIndex==4||yIndex==5));itemId='ogp-'+xIndex+'-'+yIndex;chip=ig.game.spawnEntity(EntityChip,x,y,{color:visible?color:visible,itemId:itemId,row:xIndex,col:yIndex,game:me});if(visible){visChips[itemId]=chip;}
...
return{turn:this.turn,white:whiteScore,black:blackScore}},calcScore:function(){if(window.Rvrsit&&window.Rvrsit.app){Rvrsit.app.fireEvent('scoreupdate',this,this.getScore());}},playSound:function(sound){var me=this;if(me.getSetting('fx')){if(!me.fxInitialized){this.initSound();}
...
if(nextMoves.length<1){if(numVisibleChips==me.boardSize){me.halt=true;window.Rvrsit&&Rvrsit.app.fireEvent('endgame',this,this.getScore());return;}
